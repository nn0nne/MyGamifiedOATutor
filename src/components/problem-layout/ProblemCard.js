...
class ProblemCard extends React.Component {
    ...
    constructor(props, context) {
        ...
        if (
            ...
            // Helper to safely extract first string answer
            const getFirstString = (ans) => {
                if (ans == null) return "";
                if (Array.isArray(ans)) {
                    if (ans.length === 0) return "";
                    // Recursively check each element until we find a string
                    for (let item of ans) {
                        const result = getFirstString(item);
                        if (result) return result;
                    }
                    return "";
                }
                if (typeof ans === 'object') {
                    // If it's an object, try to stringify it or look for common properties
                    return JSON.stringify(ans);
                }
                console.debug("Extracted answer:", String(ans));
                return String(ans);
            };
            // Bottom out hints
            this.hints.push({
                ...
                text: this.translate('hintsystem.answerIs'),
                showCode: true,
                code: getFirstString(this.step.stepAnswer),
                type: "bottomOut",
            ...
            this.hints.map((hint, i) => {
                if (hint.type === "scaffold") {
                    ...
                    hint.subHints.push({
                        ...
                        text: this.translate('hintsystem.answerIs'),
                        showCode: true,
                        code: getFirstString(hint.hintAnswer),
                        type: "bottomOut",
            ...
            fetch(`${MIDDLEWARE_URL}/api/piston/execute`, {
                ...
                .then(data => {
                    ...
                    this.setState({
                        ...
                        output,
                        isExecutingCode: false,
                        checkMarkOpacity: isCorrect ? "100" : "0"  // ← ADD THIS
                    });
