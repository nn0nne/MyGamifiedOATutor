...
const MIDDLEWARE_URL =
    "https://front-cancer-examinations-pike.trycloudflare.com";
...
export const findCourseByLessonId = (lessonId) => {
    for (const course of _coursePlansNoEditor) {
        const foundLesson = course.lessons.find(lesson => lesson.id === lessonId);
        if (foundLesson) {
            return course;
        }
    }
